...
class TestTransfer(SHToken):
    def test_transfer(self):
        ...
        # 赎回 10 个 token
        self.transfer(test_account, fund_account, 10*config.magnitude, config.password, 0)
        ...
        self.transfer(test_account, fund_account, 0, config.password, 0)
        self.eth_balance_of_sht_address(Web3.toWei(0.1, "ether"))

        print("start token_test 1.6.3 当打开赎回开关时，向赎回地址转账大于0个token")
        self.transfer(test_account, fund_account, 10*config.magnitude, config.password, 10*config.magnitude)
        self.eth_balance_of_sht_address(Web3.toWei(0.09, "ether"))

        print("start token_test 1.6.4 当打开赎回开关时，向赎回地址转账，转账赎回的金额大于合约预存金额")
        self.transfer(test_account, fund_account, 100*config.magnitude, config.password, 0)
        self.eth_balance_of_sht_address(Web3.toWei(0.09, "ether"))
