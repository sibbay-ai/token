...
class TestTransferByDate(SHToken):
    def test_transferByDate(self):
        ...
        self.transfer_by_date(sts.SIBBAY_SHT_OWNER, accounts[1], [10*config.magnitude], [tsec], sts.SIBBAY_SHT_PASSWORD, 10*config.magnitude)
        # 只有一个转账期限, 转账期限等于当前时间
        tsec = int(time.time())
        self.transfer_by_date(sts.SIBBAY_SHT_OWNER, accounts[1], [10*config.magnitude], [tsec], sts.SIBBAY_SHT_PASSWORD, 10*config.magnitude)
        # 只有一个转账期限, 转账期限大于当前时间
        ...
        self.transfer_by_date(sts.SIBBAY_SHT_OWNER, accounts[1], [10*config.magnitude], [tsec], sts.SIBBAY_SHT_PASSWORD, 10*config.magnitude)
        # 清空accounts[1]可用余额
        self.transfer(accounts[1], collect_account, 20*config.magnitude, config.password, 20*config.magnitude)
        ...
        self.transfer_by_date(accounts[1], accounts[2], vs, ts, config.password, 3*config.magnitude)
        ...
        self.transfer_by_date(accounts[1], accounts[2], vs, ts, config.password, 3*config.magnitude)
        # 清空accounts[1]可用余额
        self.transfer(accounts[1], collect_account, 1*config.magnitude, config.password, 1*config.magnitude)
        ...
        t = (2018, 8, 1, 10, 10, 30, 0, 0, 0)
        tsec = int(time.mktime(t))
        self.transfer_by_date(sts.SIBBAY_SHT_OWNER, accounts[1], [3*config.magnitude], [tsec], sts.SIBBAY_SHT_PASSWORD, 3*config.magnitude)
        # 转账期限都小于当前时间
        ts = [tsec, tsec + 3600]
        vs = [1*config.magnitude, 2*config.magnitude]
        self.transfer_by_date(accounts[1], accounts[2], vs, ts, config.password, 3*config.magnitude)
        ...
        self.transfer_by_date(accounts[1], accounts[2], vs, ts, config.password, 0)
        # 账期限一个小于当前时间，一个大于当前时间
        tsec = int(time.time())
        ts = [tsec - 3600, tsec + 3600]
        self.transfer_by_date(accounts[1], accounts[2], vs, ts, config.password, 0)
