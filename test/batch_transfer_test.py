...
class TestBatchTransfer(SHToken):
    def test_batch_transfer(self):
        ...
        self.batch_transfer(sts.SIBBAY_SHT_OWNER, [accounts[1]], [10*magnitude], sts.SIBBAY_SHT_PASSWORD, [10*magnitude])
        ...
        self.batch_transfer(accounts[1], [accounts[0]], [100*magnitude], password, [0])
        # 冻结账户accounts[1]
        ...
        self.batch_transfer(accounts[1], [collect_account], [0], password, [0])
        # 由账户owner向账户accounts[1]发送100个token
        self.batch_transfer(sts.SIBBAY_SHT_OWNER, [accounts[1]], [100*magnitude], sts.SIBBAY_SHT_PASSWORD, [100*magnitude])
        # 由账户accounts[1]向collect_account发送0个token
        self.batch_transfer(accounts[1], [collect_account], [0], password, [0])
        # 6.1-1.3
        ...
        self.batch_transfer(accounts[1], [collect_account], [1000*magnitude], password, [0])
        # 由账户owner向账户accounts[1]发送100个token
        self.batch_transfer(sts.SIBBAY_SHT_OWNER, [accounts[1]], [100*magnitude], sts.SIBBAY_SHT_PASSWORD, [100*magnitude])
        # 由账户accounts[1]向collect_account发送1000个token
        self.batch_transfer(accounts[1], [collect_account], [1000*magnitude], password, [0])
        # 由账户accounts[1]向collect_account发送2^256-1个token
        self.batch_transfer(accounts[1], [collect_account], [2**256-1], password, [0])
        # 6.1-1.4
        print("start token_test 6.1-1.4")
        # 由账户accounts[1]向collect_account发送10个token
        self.batch_transfer(accounts[1], [collect_account], [10*magnitude], password, [10*magnitude])
        # 6.1-1.5
        ...
        self.batch_transfer(sts.SIBBAY_SHT_OWNER, [accounts[1]], [110*magnitude], sts.SIBBAY_SHT_PASSWORD, [110*magnitude])
        # 赎回10个token
        self.batch_transfer(accounts[1], [fund_account], [10*magnitude], password, [0])
        # 打开赎回开关，并分别赎回0个，10个，100个token
        self.open_buy_sell(sts.SIBBAY_SHT_OWNER, sts.SIBBAY_SHT_PASSWORD)
        self.batch_transfer(accounts[1], [fund_account], [0], password, [0])
        self.batch_transfer(accounts[1], [fund_account], [10*magnitude], password, [0])
        self.batch_transfer(accounts[1], [fund_account], [100*magnitude], password, [0])
        # 6.1-1.7
        ...
        self.batch_transfer(accounts[1], [accounts[2], accounts[3]], [5*magnitude, 6*magnitude], password, [0, 0])
        # 向账户 accounts[2], accounts[3] 分别发送2个，3个token
        self.batch_transfer(accounts[1], [accounts[2], accounts[3]], [2*magnitude, 3*magnitude], password, [2*magnitude, 3*magnitude])
        # 向账户 accounts[2], accounts[3] 分别发送2个，3个token
        self.batch_transfer(accounts[1], [accounts[2], accounts[3]], [2*magnitude, 3*magnitude], password, [2*magnitude, 3*magnitude])
