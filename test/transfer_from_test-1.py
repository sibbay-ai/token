...
class TestTransferFrom_1(SHToken):
    def test_transfer_from_1(self):
        ...
        self.transfer_from(accounts[1], accounts[2], collect_account, 10*magnitude, password, 0)
        # 2.1-1.5
        print("start token_test 2.1-1.5")
        # 由账户accounts[2]向collect_account发送所有token
        self.transfer_from(accounts[1], accounts[2], collect_account, 90*magnitude, password, 0)
        # 2.1-1.6
        ...
        self.transfer_from(accounts[1], accounts[2], accounts[3], 100*magnitude, sts.SIBBAY_SHT_PASSWORD, 0)
