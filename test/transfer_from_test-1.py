...
class TestTransferFrom_1(SHToken):
    def test_transfer_from_1(self):
        ...
        # 2.1-1.1
        print("start token_test 2.1-1.1")
        ...
        self.unfroze(sts.SIBBAY_SHT_OWNER, accounts[2], sts.SIBBAY_SHT_PASSWORD)
        # 2.1-1.2
        print("start token_test 2.1-1.2")
        ...
        self.transfer_from(accounts[1], accounts[2], collect_account, 0, config.password, 0)
        # 2.1-1.3
        print("start token_test 2.1-1.3")
        ...
        # 2.1-1.4
        print("start token_test 2.1-1.4")
        # 由账户accounts[2]向collect_account发送10个token
        self.transfer_from(accounts[1], accounts[2], collect_account, 10*config.magnitude, config.password, 0)
        # 2.1-1.5
        print("start token_test 2.1-1.5")
        # 由账户accounts[2]向collect_account发送所有token
        self.transfer_from(accounts[1], accounts[2], collect_account, 90*config.magnitude, config.password, 0)
        # 2.1-1.6
        ...
        # 2.1-1.7
        print("start token_test 2.1-1.7")
        ...
        self.transfer_from(accounts[1], accounts[2], accounts[3], 100*config.magnitude, sts.SIBBAY_SHT_PASSWORD, 0)
